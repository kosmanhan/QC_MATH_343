...
There appears to be a lot of zero-es, which makes hurdle appropriate. It looks like a wave, with a peak at five and nine.
...
Yes, it is possible, but the computer run time will be high.
...
for (b in 1 : B){
  x_ind_b = sample(x_ind) #to sample the indices
  x_ind_b1 = x_ind_b[1 : n1]
  x_ind_b2 = x_ind_b[(n1 + 1) : n2]
  x_b1 = x[x_ind_b1]
  x_b2 = x[x_ind_b2]
  
  thetahathat_b[b] = mean(x_b1)-mean(x_b2)
}
...
```{r}
ret_a = quantile(thetahathat_b, alpha / 2)
ret_b = quantile(thetahathat_b, 1 - alpha / 2)
ggplot(data.frame(thetahathat_b = thetahathat_b)) + 
  geom_histogram(aes(x = thetahathat_b), bins = B / 100) + 
...
```{r}
2 * min(sum(thetahathat_b > thetahathat), sum(thetahathat_b < thetahathat)) / B
```
...
thetahathat_b = array(NA, B)
for (b in 1 : B){
  thetahathat_b[b] = median(sample(x2, x2len, replace = TRUE))
}
ci_a = quantile(thetahathat_b, alpha / 2)
ci_b = quantile(thetahathat_b, 1 - alpha / 2)
c(ci_a, ci_b)
```
...
```{r}
if(thetahat < ci_b && thetahat > ci_a){
  print("Fail reject H_0")
} else {
  print("Reject H_0")
}
```

As thetahathat is outside the confidence interval, we fail to reject the null hypothesis.

Use the bootstrap to create a 95% CI for `theta := Q[X_2, 0.2]` i.e. the 20th percentile where X_2 is defined as before.

```{r}
theta2hat=quantile(x2,0.2)
theta2hathat_b = array(NA, B)
for (b in 1 : B){
  theta2hathat_b[b] = quantile((sample(x2, x2len, replace = TRUE)),0.2)
}
ci_a = quantile(theta2hathat_b, alpha / 2)
ci_b = quantile(theta2hathat_b, 1 - alpha / 2)
c(ci_a, ci_b)
ci_a
ci_b
theta2hat
...
```{r}
if(theta2hat < ci_b && theta2hat > ci_a){
  print("Fail reject H_0")
} else {
  print("Reject H_0")
}
